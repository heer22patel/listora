  ...
  function attachTilt(card, maxDeg) {
    ...
    function tick() {
      ...
      card.style.transform = isHovered
        ? `translateY(-8px) perspective(900px) rotateX(${currentX}deg) rotateY(${currentY}deg)`
        : `translateY(${currentX === 0 && currentY === 0 ? '0' : (-8 * (1 - Math.max(dx,dy)/maxDeg)).toFixed(2)}px) perspective(900px) rotateX(${currentX}deg) rotateY(${currentY}deg)`;
